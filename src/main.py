...
def update_models_comparison(params, trainer, training_time_hours, test_time_hours):
    ...
    comparison_file = "E:/Documents/Teste1-1 DeepLabV3+/output/models_comparison.txt"
    ...
    with open(comparison_file, 'a', encoding='utf-8') as f:
        if not file_exists:
            ...
            f.write("=" * 120 + "\n")
            f.write(f"{'Modelo':<20} {'Data':<12} {'Val_Acc':<8} {'Val_F1':<8} {'Val_IoU':<8} {'Val_MCC':<8} ")
            f.write(f"{'Train_Acc':<8} {'Train_F1':<8} {'Train_IoU':<8} {'Train_MCC':<8} ")
        ...
        f.write(f"{best_val_acc:.4f}  {best_val_f1:.4f}  {best_val_iou:.4f}  {best_val_mcc:.4f}  ")
        f.write(f"{best_train_acc:.4f}  {best_train_f1:.4f}  {best_train_iou:.4f}  {best_train_mcc:.4f}  ")
...
if __name__=='__main__':
    ...
    # Params
    params = {
        'root_dir': r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35', # Diretório raiz dos dados
        'cache': True,
    ...
    params['results_folder'] = f"E:/Documents/Teste1-1 DeepLabV3+/output/K1x5noAug_{params['model']['name']}b45drop2_imgnet_{params['optimizer_params']['optimizer']}{params['optimizer_params']['weight_decay']}WD_{params['loss']['name']}1.0-0.5_noWeight"
    ...
    train_images1 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold1_images.txt',header=None).values
    train_images2 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold2_images.txt',header=None).values
    train_images3 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold3_images.txt',header=None).values
    train_images = [os.path.join(image_dir, f[0]) for f in np.concatenate([train_images1,train_images2,train_images3])]
    train_labels1 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold1_labels.txt',header=None).values
    train_labels2 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold2_labels.txt',header=None).values
    train_labels3 = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold3_labels.txt',header=None).values
    ...
    val_images = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold4_images.txt',header=None).values
    val_images = [os.path.join(image_dir, f[0]) for f in val_images]
    val_labels = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold4_labels.txt',header=None).values
    val_labels = [os.path.join(label_dir, f[0]) for f in val_labels ]
    
    test_images = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold5_images.txt',header=None).values
    test_images = [os.path.join(image_dir, f[0]) for f in test_images]
    test_labels = pd.read_table(r'E:\Documents\Teste1-1 DeepLabV3+\dataset_35\folds/fold5_labels.txt',header=None).values
